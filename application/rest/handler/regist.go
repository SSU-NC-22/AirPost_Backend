	...
	if node.Type[:3] == "DRO" {
		...
		for _, tag := range tags {
			...
			*/
			...
			// calc distance
			dist := registUsecase.Haversine(node.LocLat, node.LocLon, tag.LocLat, tag.LocLon)
		...
		for _, station := range stations {
			// calc distance
			dist := registUsecase.Haversine(node.LocLat, node.LocLon, station.LocLat, station.LocLon)
...
func (h *Handler) RegistDelivery(c *gin.Context) {
	...
	// The order number is always generated by the server; any client-supplied
...
func (h *Handler) GetDroneID(c *gin.Context) {
	...
	if ordernum == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "orderNum is required"})
		return
	}
	...
	// Order data is private to its owner (or an admin).
	if !requireOwnerOrAdmin(c, delivery.Email) {
		return
	}
...
func (h *Handler) GetTracking(c *gin.Context) {
	...
	// Tracking is private to the delivery's owner (or an admin).
	if !requireOwnerOrAdmin(c, delivery.Email) {
		return
	}
